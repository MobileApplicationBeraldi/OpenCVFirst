...
object OpenCV {
    ...
    fun AR(m:Mat,context:Context){

        Log.i("INIT", "Try load classifier...")
        // load cascade file from application resources
        val ins = context.resources.openRawResource(R.raw.haarcascade_eye)
        ...
        val eye_detector = CascadeClassifier(cascadeFile.getAbsolutePath())
        ...
        val mor = MatOfRect()
        eye_detector.detectMultiScale(m,mor,1.1)
        val i = mor.size()
        Toast.makeText(context,""+i,Toast.LENGTH_SHORT).show()
        val facesArray: Array<Rect> = mor.toArray()
        for (i in facesArray.indices) rectangle(
            m,
            facesArray[i].tl(),
            facesArray[i].br(),
            Scalar(100.0,0.0,0.0,255.0),
            1
        )
    }
